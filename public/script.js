...
function renderCard(item) {
  ...
  const chatBtnHtml = (item.ownerUid && !isOwner && (!currentUserForChat || currentUserForChat.uid !== item.ownerUid))
    ? `<button class="btn-share" onclick="try{startChatWith({itemType:'item', itemId:'${item.id}', itemName:'${escapeHtml(item.name).replace(/'/g, "\\'")}', otherUid:'${item.ownerUid}'})}catch(e){alert('خطأ بالمحادثة: '+e.message)}">💬 محادثة</button>`
    : '';
